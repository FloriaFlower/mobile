if (typeof window.YuseTheaterApp === 'undefined') {
   window.YuseTheaterRegex = {
     ...
     // 1. 通告：7个字段（announcement|id|标题|演员|地点|报酬|描述）→7个捕获组
     announcement: /announcement\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)/g,
     // 2. 定制：8个字段（customization|id|粉丝ID|类型|截止时间|报酬|需求|备注）→8个捕获组
     customization: /customization\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)/g,
     // 3. 剧场：10个字段（theater|id|标题|封面|描述|热度|收藏|观看|价格|评论）→10个捕获组
     theater: /theater\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)/g,
     // 4. 商品：7个字段（shop|id|名称|描述|基础价|最高价|评论）→7个捕获组
     shop: /shop\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)\|([^\|]+)/g
   };
    ...
    parseNewData() {
      ...
      try {
        ...
        if (fullMatch) {
          ...
          // ========== 1. 解析“通告”：新增字符串清理（解决只显示一栏） ==========
          const parseAnnouncements = (str) => {
            if (!str) return '<div class="empty-state">暂无通告</div>';
            // 修复：清理字符串（去掉首尾换行、合并多余空格），确保正则匹配所有条目
            const cleanStr = str.replace(/^[\n\r]+|[\n\r]+$/g, '').replace(/\n+/g, '\n').trim();
            let html = '';
            // 用清理后的字符串匹配所有通告条目
            cleanStr.replace(window.YuseTheaterRegex.announcement, (match, id, title, actor, location, payment, description) => {
              html += `
                <div class="list-item" 
            ...
            console.log('[YuseTheater] 通告解析结果（含多栏）:', html); // 验证多栏生成
            return html;
          };
          // ========== 2. 解析“粉丝定制”：新增字符串清理（解决只显示一栏） ==========
          const parseCustomizations = (str) => {
            if (!str) return '<div class="empty-state">暂无定制</div>';
            // 修复：清理字符串
            const cleanStr = str.replace(/^[\n\r]+|[\n\r]+$/g, '').replace(/\n+/g, '\n').trim();
            let html = '';
            cleanStr.replace(window.YuseTheaterRegex.customization, (match, id, fanId, typeName, deadline, payment, request, notes) => {
              html += `
                <div class="list-item" 
            ...
            console.log('[YuseTheater] 定制解析结果（含多栏）:', html); // 验证多栏生成
            return html;
          };
          // ========== 3. 解析“剧场”：新增字符串清理（解决只显示一栏） ==========
          const parseTheater = (str) => {
            if (!str) return '<div class="empty-state">暂无剧场</div>';
            // 修复：清理字符串
            const cleanStr = str.replace(/^[\n\r]+|[\n\r]+$/g, '').replace(/\n+/g, '\n').trim();
            let html = '';
            cleanStr.replace(window.YuseTheaterRegex.theater, (match, id, title, cover, description, popularity, favorites, views, price, reviews) => {
              html += `
                <div class="list-item" 
                     data-type="theater" 
                     data-id="${id || '-'}" 
            ...
            console.log('[YuseTheater] 剧场解析结果（含多栏）:', html); // 验证多栏生成
            return html;
          };
          // ========== 4. 解析“商品”：新增字符串清理（解决只显示一栏） ==========
          const parseShop = (str) => {
            if (!str) return '<div class="empty-state">暂无商品</div>';
            // 修复：清理字符串
            const cleanStr = str.replace(/^[\n\r]+|[\n\r]+$/g, '').replace(/\n+/g, '\n').trim();
            let html = '';
            cleanStr.replace(window.YuseTheaterRegex.shop, (match, id, name, description, price, highestBid, comments) => {
              html += `
                <div class="list-item" 
                     data-type="shop" 
                     data-id="${id || '-'}" 
            ...
            console.log('[YuseTheater] 商品解析结果（含多栏）:', html); // 验证多栏生成
            return html;
          };
          // ========== 5. 赋值解析后的HTML，渲染页面 ==========
...
showItemDetail(itemData) {
  ...
  switch (itemData.type) {
    ...
    case 'theater':
      ...
      `;
      // 解析评论：修复JSON格式（清理换行/多余空格）
      if (itemData.reviews) {
        try {
          // 关键修复：清理JSON字符串（去掉换行、合并空格）
          const cleanReviews = itemData.reviews
            .replace(/[\n\r]/g, '') // 移除所有换行符
            .replace(/\s+/g, ' ')   // 多余空格合并为1个
            .trim();                 // 去掉首尾空格
          const reviews = JSON.parse(cleanReviews);
          ...
          console.warn('[YuseTheater] 解析评论失败:', e, '原始数据:', itemData.reviews);
        }
      }
      break;
    case 'shop':
      ...
      `;
      // 解析商品评论：修复JSON格式（清理换行/多余空格）
      if (itemData.comments) {
        try {
          // 关键修复：清理JSON字符串
          const cleanComments = itemData.comments
            .replace(/[\n\r]/g, '')
            .replace(/\s+/g, ' ')
            .trim();
          const comments = JSON.parse(cleanComments);
          ...
          console.warn('[YuseTheater] 解析商品评论失败:', e, '原始数据:', itemData.comments);
        }
      }
      break;
  }
